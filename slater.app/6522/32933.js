...
function initSplitText() {
  ...
  $('[data-split-text="words"] .single-word').wrapInner(
...
function initCheckTheme() {
  function checkThemeSection() {
    ...
    for (var i = 0; i < themeSections.length; i++) {
      ...
      var themeObserverOffset =
        $(".main-nav-bar .nav-bar-links.right").innerHeight() / 2;
      ...
      if (
        ...
        if ($("[data-id-nav]").attr("data-id-nav") == idSectionActive) {
        } else {
  ...
  $("[data-marquee-target]").each(function () {
    ...
    if (marquee.attr("data-marquee-duplicate")) {
      ...
      for (var i = 0; i < marqueeDuplicateAmount; i++) {
        var clonedMarqueeContent = marquee
          .find(".marquee-content")
          .first()
          .clone();
...
function initScrolltriggerAnimations() {
  ...
  $(".row-westoost-arrows").each(function () {
    let triggerElement = $(this);
    let targetElementOriginal = $(this).find(
      ".svg-vector-line-curl-left.original path"
    );
    let targetElementDuplicate = $(this).find(
      ".svg-vector-line-curl-left.duplicate path"
    );
